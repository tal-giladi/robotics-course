"""20.02 — the integration arithmetic: wire, fuse, and where the mass ended up.
...
* ``tipping_decel``    — the braking that puts the robot on its nose.
...
def tipping_decel(x_cg_m: float, z_cg_m: float) -> float:
    """Braking deceleration (m/s^2) at which karmel pivots forward over its wheel axle.

    karmel touches the ground at two drive wheels on the axle (x = 0) and one caster *behind*
    them, so there is nothing holding the nose down in front. Braking makes inertia act forward
    at the CG height; the robot tips when that moment beats gravity's:

    $$m\\,a\\,z_{cg} > m\\,g\\,|x_{cg}| \\quad\\Rightarrow\\quad a_\\text{tip} = \\frac{g\\,|x_{cg}|}{z_{cg}}$$

    Return ``0.0`` when ``x_cg_m >= 0`` (the CG is already on or in front of the axle: the robot
    is resting on its nose, and any braking at all tips it) and when ``z_cg_m <= 0``.

    >>> round(tipping_decel(-0.027, 0.070), 2)
    3.78
    >>> tipping_decel(0.003, 0.074)
    0.0
    """
    raise NotImplementedError  # TODO(student)
