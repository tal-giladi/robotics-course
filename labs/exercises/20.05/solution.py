"""20.05 — turning runs into verdicts (reference solution), and two suites into a regression report.
...
def verdict(run: Mapping[str, Any], thresholds: Mapping[str, Any]) -> list[str]:
    ...
    if outcome == "reach" and not run["reached"]:
        broken.append("not-reached")
    if outcome == "stop-safely":
        if run["reached"]:
            broken.append("should-stop")
        elif run["interventions"] == 0:
            broken.append("lucky-stop")
    if run["collided"]:
        broken.append("collided")
    if run["duration_s"] > thresholds["max_time_s"]:
        broken.append("too-slow")
    if run["min_clearance_m"] < thresholds["min_clearance_m"]:
        broken.append("too-close")
    if run["interventions"] > thresholds["max_interventions"]:
        broken.append("too-many-stops")
    if run["blind_distance_m"] > thresholds["max_blind_distance_m"]:
        broken.append("drove-blind")
...
def regressions(before: Mapping[str, Mapping[str, Any]], after: Mapping[str, Mapping[str, Any]],
    ...
    * ``"WORSE <id>: time"`` — ``duration_s`` grew by more than 25 % **and** by more than 1 s
    ...
    for sid in sorted(set(before) & set(after)):
        limits = thresholds[sid]
        old, new = before[sid], after[sid]
        old_verdict, new_verdict = verdict(old, limits), verdict(new, limits)
        if not old_verdict and new_verdict:
            lines.append(f"REGRESSION {sid}: {', '.join(new_verdict)}")
        elif old_verdict and not new_verdict:
            lines.append(f"FIXED {sid}")
        if new["min_clearance_m"] < old["min_clearance_m"] - 0.02:
            lines.append(f"WORSE {sid}: clearance")
        if new["duration_s"] > old["duration_s"] * 1.25 and new["duration_s"] > old["duration_s"] + 1.0:
            lines.append(f"WORSE {sid}: time")
    return lines
