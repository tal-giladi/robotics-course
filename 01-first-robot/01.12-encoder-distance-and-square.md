...
2. **It slows down near the goal.** Without the ramp the wheels coast past the target by several
   percent; in the simulator, removing it turns a 1369-tick turn into a 1480-tick turn — 7° of
   overshoot at every corner, which is 26° after four.
...
| Stopping tolerance and coasting | 0.5 % per segment | systematic | better control, [08.07](../08-control/08.07-pid-together.md) |
...
6. Why does `move()` slow down over the last 10 cm instead of driving at full speed and then braking?
   ...
   keeps travelling. Arriving slowly makes that residual tiny. In the simulator, removing the ramp
   turns a 1369-tick turn into roughly 1480 ticks — 7° of overshoot per corner, 26° over a square.
   </details>
